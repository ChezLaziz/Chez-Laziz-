...
export default function NotFoundPage() {
  ...
  const isAr = lang === 'ar'
  useSEO({
    title: isAr ? 'الصفحة غير موجودة — Chez Laziz' : 'Page introuvable — Chez Laziz',
    description: isAr
    ...
    <div className="min-h-screen bg-[#faf6f3]">
      ...
      <main className="mx-auto flex max-w-2xl flex-col items-center px-5 py-32 text-center md:py-44">
        <p className="mb-5 text-[11px] font-medium uppercase tracking-[0.35em] text-accent">
          {isAr ? 'خطأ 404' : 'Erreur 404'}
        </p>
        <h1 className="font-display text-4xl leading-tight md:text-6xl">
          {isAr ? 'هذه الصفحة غير موجودة' : "Cette page n'existe pas"}
        </h1>
        <p className="mt-5 max-w-md text-[15px] font-light leading-relaxed text-ink/70">
          {isAr
            ? 'قد يكون الرابط خاطئًا أو تم نقل الصفحة. أمّا مقروضنا، فهو دائمًا في مكانه.'
            : 'Le lien est peut-être erroné ou la page a été déplacée. Nos makroudh, eux, sont toujours là.'}
        </p>
        <div className="mt-10 flex flex-col gap-4 sm:flex-row">
          ...
          >
            {isAr ? 'شاهدوا التشكيلة' : 'Voir la collection'}
          </Link>
          <Link
            to={isAr ? '/ar' : '/'}
        ...
        <p className="mt-10 text-sm font-light text-ink/50">
          {isAr ? (
            <>
              لديكم سؤال؟{' '}
              <a href={PHONE_TEL} dir="ltr" className="text-accent underline underline-offset-2">
                {PHONE_DISPLAY}
              </a>
            </>
          ) : (
            <>
              Une question ?{' '}
              <a href={PHONE_TEL} className="text-accent underline underline-offset-2">
                {PHONE_DISPLAY}
              </a>
            </>
          )}
