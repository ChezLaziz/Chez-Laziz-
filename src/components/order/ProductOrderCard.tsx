...
export default function ProductOrderCard({
  ...
  return (
    <article
      ...
      aria-label={`${product.name} — ${formatPriceDT(product.priceMillimes)} le kilo`}
    >
      ...
      <div className="flex flex-1 flex-col p-3.5 md:p-4">
        ...
        <p className="mt-1.5 text-[11px] uppercase tracking-[0.16em] text-ink/50">
          {formatPriceDT(product.priceMillimes)} / kg
        </p>

        <label className="mt-3 block">
          <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-ink/50">Poids</span>
          <select
            value={weight}
          ...
          >
            {ALLOWED_WEIGHTS_KG.map((w) => (
              <option key={w} value={w}>
                {formatWeight(w)} — {formatPriceDT(priceForWeight(product.priceMillimes, w))}
              </option>
            ))}
          </select>
        </label>
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-x-2 gap-y-3">
          <span className="whitespace-nowrap font-display text-lg text-accent">{formatPriceDT(price)}</span>
          {qty === 0 ? (
            <button
              type="button"
            ...
            <div className="flex items-center gap-1.5" role="group" aria-label={`Quantité — ${product.name} ${formatWeight(weight)}`}>
              <button type="button" aria-label={`Retirer un ${product.name}`} onClick={() => onSetQty(weight, qty - 1)} className={stepperBtnCls}>
                −
              </button>
              <span className="w-7 text-center font-display text-lg" aria-live="polite">
                {qty}
              </span>
              <button type="button" aria-label={`Ajouter un ${product.name}`} onClick={() => onSetQty(weight, qty + 1)} className={stepperBtnCls}>
                +
              </button>
            </div>
          )}
        ...
        {otherLines.length > 0 && (
          <p className="mt-2 text-[11px] text-ink/55">
            ...
            {otherLines.map((w) => `${qtyByWeight[w]} × ${formatWeight(w)}`).join(', ')}
          </p>
        )}
      </div>
    </article>
